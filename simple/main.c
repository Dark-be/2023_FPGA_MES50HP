...
void lcd_draw_char(uint16_t x, uint16_t y, uint8_t ch, uint16_t color, uint8_t size)
{
    ...
    uint8_t *p;
    if(ch>='A' && ch<='Z')
        p = (uint8_t*)&font[(ch-'A')*4+3][15];
    else if(ch>='a' && ch<='z')
        p = (uint8_t*)&font[(ch-'a')*4+3+104][15];
    else if(ch>='0' && ch<='9')
        ;
    else return;
    for (i=0;i<16; i++)
...
void ui_draw_icon(uint8_t num){
    if(num==0){
        lcd_draw_rect(LCD_HEIGHT/2-40, LCD_WIDTH/2-40, 80, 80, COLOR_GREEN);//background
        lcd_draw_char(LCD_HEIGHT/2-16, LCD_WIDTH/2-20, 'G', COLOR_YELLOW,2);
        lcd_draw_string(LCD_HEIGHT/2-41, LCD_WIDTH/2-40, "SNAKE", COLOR_RED,1);
    }
    else if(num==1){
...
void ui_draw_scene(uint8_t id){
    if(id==0){
        ...
        lcd_draw_string(LCD_HEIGHT/2-134, LCD_WIDTH/2-100, "Press UP to play", COLOR_WHITE,1);
        ...
        if(data != 0){
            ...
            if(state==0){
                if(data == COMMAND_A || data == COMMAND_D){
                    if(data == COMMAND_A){
                        if(menu_choice==0) menu_choice=GAME_COUNT;
                        else menu_choice--;
                    }
